...
function renderHome(){layout('DAMOLDA'); $('.app').innerHTML=`<section class="hero"><img class="hero-bg" src="assets/hero.png" alt=""><div class="hero-copy"><h1>조용한 깃듦</h1><p>잔상이 고요하게 머물 수 있도록</p><a class="linkline" href="${href('themes.html')}">${t[lang].enterThemes} →</a></div></section>`}
...
function renderAbout(){layout(`${t[lang].about} - DAMOLDA`); const bodyKo=`<p>어떤 순간들은 앞으로도 이어질 가치가 있습니다.</p><p>드물어서가 아니라, 거창해서가 아니라, 우리 삶의 일부가 되었기 때문입니다.</p><p>여행의 조각, 서랍 속 편지, 사랑하는 존재의 작은 흔적, 평범한 하루에 남은 물건.</p><p>DAMOLDA는 그런 순간들을 위한 형태를 생각합니다. 과거를 그대로 붙잡기 위해서가 아니라, 일상 속에 둘 수 있는 모습으로 남기기 위해서입니다.</p><p>기억은 이야기의 일부일 뿐입니다. 우리가 간직하기로 한 것들은 지나간 일을 떠올리게 하는 데서 그치지 않습니다. 그것들은 다음에 올 시간과 함께 놓입니다.</p><p>DAMOLDA는 당신의 모든 시간이 한 장소에 깃들 수 있도록 존재합니다.</p>`; const bodyEn=`<p>잊혀지기 전 새겨질 수 있도록</p><p>Not because they are rare. Not because they are grand. But because they become part of who we are.</p><p>A ticket from a journey. A letter kept in a drawer. A small trace of a beloved companion. An object from an ordinary day.</p><p>DAMOLDA exists for those moments. Not to preserve them exactly as they were, but to give them a form that can remain in everyday life.</p><p>Memory is only part of the story. The objects we keep are not only reminders of what happened. They become companions to what comes next.</p><p>DAMOLDA는 당신의 모든 시간이 한 장소에 깃들 수 있도록 존재합니다.</p>`; $('.app').innerHTML=`<main class="page about-shell"><div class="section-head"><h1 class="page-title">${t[lang].about}</h1><p>DAMOLDA</p></div><section class="about-panel"><div class="about-title"><div class="kicker">DAMOLDA</div><h1>잊혀지기 전 새겨질 수 있도록</h1></div><div class="about-text">${lang==='ko'?bodyKo:bodyEn}</div></section></main>`}
