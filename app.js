...
function renderJournal(){layout(`${t[lang].journal} - DAMOLDA`); const cards=[t[lang].making,t[lang].materials,t[lang].process,t[lang].studio]; $('.app').innerHTML=`<main class="page"><div class="section-head"><h1>${t[lang].journal}</h1><p>Studio Archive</p></div><div class="journal-grid">${cards.map(c=>`<article class="journal-card"><div class="kicker">Coming Soon</div><h2>${c}</h2><p>${t[lang].soonText}</p></article>`).join('')}</div></main>`}
