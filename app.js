...
function renderJournal(){layout(`${t[lang].journal} - DAMOLDA`); const cards=[{title:t[lang].making,img:'assets/journal-workshop-bg.png'},{title:t[lang].materials,img:'assets/theme-stage-bg.png'},{title:t[lang].process,img:'assets/journal-workshop-bg.png'},{title:t[lang].studio,img:'assets/theme-stage-bg.png'}]; $('.app').innerHTML=`<main class="page journal-shell journal-bg"><div class="journal-hero"><h1>${t[lang].journal}</h1><p>다몰다의 생각과 과정, 그리고 일상의 기록들</p></div><div class="journal-grid">${cards.map(c=>`<article class="journal-card action-card"><div class="card-thumb" style="background-image:url('${c.img}')"></div><div class="card-body"><div class="kicker">Coming Soon</div><h2>${c.title}</h2><p>${t[lang].soonText}</p></div></article>`).join('')}</div></main>`}
